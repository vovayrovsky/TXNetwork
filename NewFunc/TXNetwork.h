...
//{
    ...
    //! @brief   Cокет TXLib'a.
    //!
    //! @param   to        Сокет, через который нужно принять данные.
...
/*    //! @ingroup TXSocket
    ...
    //! @brief   Cокет TXLib'a.
    //!
    //! @param   to        Сокет, очередь приема которого нужно очистить
    //!
    //! @return  Код ошибки. Подробнее см. MSDN.
    //! @par     Пример 1
    //! @code
    ...
    int txCleanUpQueue (TX_SOCKET& to);*/
    ...
    //! @brief   Cокет TXLib'a.
    //!
    //! @param   to      Сокет, размер входящего буфера которого нужно установить.
    //! @param   size_   Новый размер входящего буфера
    //! @param   oldsize Указатель на переменную, в которую будет положен старый размер буфера
    //!
    //! @return  Код ошибки.
